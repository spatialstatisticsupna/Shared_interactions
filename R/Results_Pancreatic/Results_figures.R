...
################################################################################
###########      Figure 5: Posterior median of the area-specific     ###########
...
p <- ggplot(data=data.temporal, aes(x=ID_time, y=median, group=type)) + 
  ...
  annotate("text", x=9, y=alpha.I - 0.3, size=7, 
  ...
  annotate("text", x=9, y=alpha.M - 0.3, size=7, 
...
pdf("./Figures/Figure5.pdf", height=13, width=24)
library(ggpubr)
...
################################################################################
###########   Figure 6: Posterior medians and 95% credible interval  ###########
...
spattemp.inc.mean <- lapply(1:n, function(l) sapply(1:t, function(z) round(inla.est.inc[[l]][[z]]$mean,3)))
spattemp.mort.mean <- lapply(1:n, function(l) sapply(1:t, function(z) round(inla.est.mort[[l]][[z]]$mean,3)))
...
spattemp.mort <- NULL
for (z in 1:t) {
  spattemp.inc <- c(spattemp.inc,sapply(1:n, function(l) spattemp.inc.mean[[l]][z]))
  spattemp.mort <- c(spattemp.mort,sapply(1:n, function(l) spattemp.mort.mean[[l]][z]))
}
...
library(ggplot2)
pdf("./Figures/Figure6.pdf", height=6, width=10)
...
################################################################################
###########    Figure C1: Posterior median of the evolution of the   ###########
...
break_points <- c(round(seq(round(min(df$fnl_rts)-1,0), 
                            round(max(df$fnl_rts)+1,0), length.out = 11),0))

cols <- c("#fcfdbf", "#fecf92", "#fe9f6d", "#f7705c", "#de4968", "#b73779",
          "#8c2981", "#641a80", "#3b0f70", "#140e36")
...
pdf("./Figures/FigureC1.pdf", height=28, width=15)
library(ggpubr)
ggarrange(s1, s2,
          ncol = 1, nrow = 2)
dev.off()
